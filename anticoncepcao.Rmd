...
```{r}
'
#corrigindo o problema com o pacote caret
...
A população em estudo consiste de r count(unique(anticoncepcao_13$USUARIO)) mulheres em 2013, r count(unique(anticoncepcao_14$USUARIO)), em 2014, r count(unique(anticoncepcao_15$USUARIO)) em 2015, r count(unique(anticoncepcao_16$USUARIO)), em 2016 e r count(unique(anticoncepcao_17$USUARIO)) em 2017. Esta população foi analisada em relação à idade, escolaridade e cor da pele, como descrito a seguir.
...
##Recategorização Como o número de participantes em algumas categorias é muito pequeno, as variáveis foram recategorizadas da seguinte forma: Cor da Pele: Branca, Parda, Preta, Outras (Amarela e Indígena) Escolaridade: Alta (Doutorado, Mestrado, Especialização/Residencia, Superior completo e Superior incompleto), Média (Médio completo (2° grau) e Médio incompleto (2° grau)) e Baixa(Fundamental completo (1° grau) e Fundamental incompleto (1° grau)). Como a taxa de analfabetismo em Florianópolis é baixa e a escolaridade média é a maior entre as capitais, é pouco provável que os elevados números de mulheres nas categrias Alfabetizado e Não sabe ler / escrever estejam corretos, principalmente para a faixa etária analizada (15 a 49 anos), uma vez que a maior quantidade de pessoas com baixa escolarização em adultos encontra-se entre os mais velhos. Desta forma, as informações sobre estas categorias não foram levadas em consideração.
...
##Análise das Prescrições e da Inserção de DIU A distribuição das prescrições de métodos anticoncepcionais e a implantação do DIU foi analisada de acordo com os Centros de Saúde, áreas de abrangência das equipes de saúde da família e profissional prescritor.
...
##Métos prescritos na atenção primária de Florianópolis Para esta análise, foram extraídos dados de prescrição de métodos anticoncepcionais e de inserção de DIU, na atenção primária de Florianópolis, no período de 01 de janeiro de 2013 a 31 de dezembro de 2017. As solicitações de laquedura e vazectomia não foram avaliadas.11
...
```{r}
#REGRESSÂO LOGÍSTICA
rl_diu <- glm(USO_DIU ~ IDADE + RACA_COR_GRUPO + ESPECIALIDADE_GRUPO + UNIDADE_AREA, data = train_prep, family = "binomial") 
print(summary(rl_diu))

#train_fit <- glm(USO_DIU ~ IDADE + ESPECIALIDADE_GRUPO + UNIDADE_AREA + RACA_COR_GRUPO + ESCOL_GRUPO, data = train_prep, family=binomial(link='logit'))

#knitr::kable(coefficients(summary(rl_diu)),
#             align = c('r'), caption = "Regressão logistica: USO_DIU e Todas Variáveis", "html") %>% kable_styling(full_width = T)
#anova(rl_diu, test="Chisq")

train_prep <- train[,c(9,17,18,19,21,22)]

#REGRESSÂO LOGÍSTICA
rl_diu <- glm(USO_DIU ~ IDADE + RACA_COR_GRUPO + ESPECIALIDADE_GRUPO + UNIDADE_AREA + ESCOL_GRUPO, data = train_prep, family = "binomial") 
print(summary(rl_diu))
test_prep <- test[,c(9,17,18,19,21,22)]

test_probs <- predict(rl_diu,test_prep,type='response')
test_fit <- glm(USO_DIU ~ IDADE + RACA_COR_GRUPO + ESPECIALIDADE_GRUPO + UNIDADE_AREA + ESCOL_GRUPO, data = test_prep, family=binomial(link='logit'))
#knitr::kable(coefficients(summary(test_fit)),
#             align = c('r'), caption = "Regressão logistica: USO_DIU e Todas Variáveis", "html") %>% kable_styling(full_width = T)
hist(test_probs)
anova(test_fit, test="Chisq")
...
```{r}
#diu_sim <- subset(anticoncepcao_t, anticoncepcao_t$USO_DIU == "sim")
#diu_nao <- subset(anticoncepcao_t, anticoncepcao_t$USO_DIU == "não")


balanced_data <- ovun.sample(USO_DIU ~ ., data = anticoncepcao_t, method = "over",N = 182000)$data
table(balanced_data$USO_DIU)
```

```{r}
'
####Regressão Logistica com Cross Validation e cálculo da AUC
# Train glm with custom trainControl: model
model_reglog <- train(USO_DIU ~ IDADE + ESPECIALIDADE_GRUPO + ESCOL_GRUPO + RACA_COR_GRUPO,
               metric = "ROC",
               method = "glm",
               data = anticoncepcao_t,
...
```{r}
'
#Random forest com cross validation e tuneLength =3

#levels(anticoncepcao_t) <- c("first_class", "second_class")

# Fit random forest: model
model_rf <- train(
  USO_DIU ~ .,
  #preProcess = "medianImpute", #imputado valores nas células NA quando se trabalhar com variáveis contínulas
  tuneLength = 3,
  data = anticoncepcao_t,
  metric = "ROC",
  method = "ranger",
...
'
# Create model_list
model_list <- list(regLog = model_reglog, glmnet = model_glmnet, rf = model_rf)


# Pass model_list to resamples(): resamples
resamples <- resamples(model_list, measures = list(tpr, fpr, fnr, acc), show.info = T)
...
```{r}
'
#anticoncepcao_t$USO_DIU <- as.factor(anticoncepcao_t$USO_DIU)
#anticoncepcao_t <- na.omit(anticoncepcao_t)


############## dividir o processamento nos cores da CPU ##############
#bibliotecas usadas: library(parallel) e library(snow)

#cria clusters com a quantidade de cores
cl <- makeCluster(detectCores())
registerDoParallel(cl)

#roda o código nos clusters
#sub <- c(sample(1:26000, 7), sample(26001:52000, 7), sample(52001:78000, 7))
anticoncepcao.bagging <- bagging(USO_DIU~., data=anticoncepcao_t, mfinal=3)
anticoncepcao.bagging <- predict.bagging(anticoncepcao.bagging, newdata=anticoncepcao_t[-sub,])
anticoncepcao.bagging

#para o processamento nos clusters
stopCluster(cl)

l <- length(anticoncepcao_t[,1])

sub <- sample(1:l,2*l/3)
BC.bagging <- bagging(USO_DIU ~.,data=anticoncepcao_t[,-1],mfinal=5, 
control=rpart.control(maxdepth=3))
BC.bagging.pred <- predict.bagging(BC.bagging,newdata=anticoncepcao_t[-sub,-1])
